...
export function WorkloadWidget({ projects }: WorkloadWidgetProps) {
  ...
  const maxLoad = Math.max(...loadList.map(l => l.totalLoad), 1);
              ...
              <div key={engineer.id} className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{totalLoad} <span className="text-xs text-gray-500 font-normal">pts</span></p>
                  </div>
                </div>
                
                <div className="w-full bg-gray-100 dark:bg-gray-800 rounded-full h-1.5 mb-1 overflow-hidden flex">
                  <div 
                    className="bg-indigo-500 h-1.5 rounded-l-full" 
                    style={{ width: \`\${(activeProjects / maxLoad) * 100}%\` }}
                    title={\`\${activeProjects} active projects\`}
                  ></div>
                  <div 
                    className="bg-blue-400 h-1.5" 
                    style={{ width: \`\${(projectTasks / maxLoad) * 100}%\` }}
                    title={\`\${projectTasks} project tasks\`}
                  ></div>
                  <div 
                    className="bg-purple-400 h-1.5 rounded-r-full" 
                    style={{ width: \`\${(delegatedTasks / maxLoad) * 100}%\` }}
                    title={\`\${delegatedTasks} delegated tasks\`}
                  ></div>
                </div>
...
fs.writeFileSync('src/components/dashboard/WorkloadWidget.tsx', newCode);
