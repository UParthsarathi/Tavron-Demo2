...
const logsTab = `import React from 'react';
...
interface ProjectLogsTabProps {
  project: Project;
  activeLogTab: string;
  onSetActiveLogTab: (tab: string) => void;
}

export function ProjectLogsTab({ project, activeLogTab, onSetActiveLogTab }: ProjectLogsTabProps) {
  return (
    <div>
            ...
            {logEngineers.length > 0 && (
              <div className="flex flex-wrap gap-2">
                ...
                {logEngineers.map(eng => (
                  <button
                    key={eng.id}
                    onClick={() => onSetActiveLogTab(eng.id)}
                    className={cn(
                      "px-4 py-1.5 rounded-full text-sm font-medium transition-colors border",
                      activeLogTab === eng.id 
                        ? "bg-gray-900 dark:bg-white text-white dark:text-gray-900 border-gray-900 dark:border-white" 
...
`;
fs.writeFileSync('src/components/projects/tabs/ProjectLogsTab.tsx', logsTab);
